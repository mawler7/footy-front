  ...
  useEffect(() => {
    ...
    Object.entries(groupedStandings)?.forEach(([groupName, teams]) => {
      teams.forEach((team) => {
        ...
        console.log('Team filteredData:', team.filteredData);
      });
    });
        ...
        <FilterContainer>
          {leagues
            ...
            .map((league) => (
              ...
              >
                <LeagueLogoImage src={league.league.logo} alt={league.league.leagueName} />
              ...
              <StyledTableContainer $isBettingSlipOpen={isBettingSlipOpen}>
                <thead>
                  <StyledTableRow>
                    ...
                    <StyledTableHeader className="form-header" onClick={() => handleSort('formPoints')}>
                      Form
                    </StyledTableHeader>
                  </StyledTableRow>
                </thead>
                <tbody>
                  {sortedTeams(teams).map((team, index) => {
                    const goalsData = team.filteredData?.goals || { for: 0, against: 0 };
                    const goalsFor = goalsData.for ?? 0;
                    const goalsAgainst = goalsData.against ?? 0;
                    return (
