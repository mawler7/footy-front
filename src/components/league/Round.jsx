...
const Round = ({ round, matches, expanded, onToggle, onMatchClick, formatDateTime }) => (
    <RoundWrapper>
        ...
        {expanded && matches.length > 0 && matches.map((match) => {
            const isHomeWinner = match.home > match.away;
            const isAwayWinner = match.away > match.home;
            return (
                <TableRow key={match.id} onClick={() => onMatchClick(match.id)}>
                    <DateCell>{formatDateTime(match.date)}</DateCell>
                    <HomeTeamCell>
                        <TeamName $align="right" $isWinner={isHomeWinner}>{match.homeTeamName}</TeamName>
                    ...
                    <AwayTeamCell>
                        ...
                        <TeamName $align="left" $isWinner={isAwayWinner}>{match.awayTeamName}</TeamName>
                    </AwayTeamCell>
                </TableRow>
            );
        })}
