...
const Events = ({
    ...
    const renderEventItems = (events, teamName) => {
        if (!events.length) {
            return;
        }
        return events.map((event, index) => (
            <EventItem key={index} isHome={event.team.name === teamName}>
                {event.team.name === teamName ? (
                    <>
                        <EventTime $isHome={event.team.name === teamName}>{event.time.elapsed} {event.time.extra ? `+${event.time.extra}'` : "'"}</EventTime>
                        <EventIcon $isHome={event.team.name === teamName}>{getEventIcon(event)}</EventIcon>
    ...
    return (
        <EventsSection isBettingSlipOpen={isBettingSlipOpen}>
