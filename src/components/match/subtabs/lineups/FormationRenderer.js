...
const FormationRenderer = ({
    ...
    if (!team?.startXI || !team?.formation) {
        return null;
    }

    const renderGoalkeeper = () => {
        if (!goalkeeper || !goalkeeper.player || !goalkeeper.player.id) {
            console.warn('Goalkeeper data is missing or invalid:', goalkeeper);
            return null;
        }
    ...
    const renderPlayers = () =>
        lines.map((line, lineIndex) =>
            line.map((playerData, playerIndex) => {
                ...
                const posX = getHorizontalPosition(
                    lineIndex,
                    lines.length,
                ...
                const posY = getVerticalPosition(
                    playerIndex,
                    line.length,
